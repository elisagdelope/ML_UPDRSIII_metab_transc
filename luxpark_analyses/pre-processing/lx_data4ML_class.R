...
# Usage: Rscript lx_data4ML_class.R 
...
p <- add_argument(parser = p, arg = "--analysis", help = "name of analysis directory (e.g., 02-pred-V0-PD, 02-pred-V0-UPDRS3-class)", required = TRUE)
...
for (e_level in c("METAB", "PW")) {
  if (e_level == "METAB") {
    ...
    process_data4ML(METAB.FILE, PHENO.FILE, features_varname, OUT_DIR_DATA, e_level, st, target, myseed, export=TRUE)
  } else { # aggregationss
